...
private:
  struct Node
  {
    ...
    {
      if (this->get_parent(version, nullptr) == nullptr)
        return nullptr;
    ...
    {
      if (this->get_grandparent(version) == nullptr)
        return nullptr;
      if (this->get_parent(version, nullptr)->is_left_child(version))
        return this->get_grandparent(version)->get_right(version, nullptr);
  ...
  void insert_fix(Node *node, int version)
  {
    if (node == nullptr)
      return;
    if (node->get_parent(version, nullptr) != nullptr && node->get_parent(version, nullptr)->get_color(version, BLACK) == COLOR::BLACK)
      return;
  ...
  Node *search_helper(Node *node, int value, int version)
  {
    if (node->is_null)
      return nullptr;
    if (node->value > value)
      return search_helper(node->get_left(version, nullptr), value, version);
    else if (node->value < value)
      return search_helper(node->get_right(version, nullptr), value, version);
    return node;
  }
...
public:
  ...
  public:
    ...
    Node *successor(Node *n, int version)
    {
      if (n == nullptr)
        return nullptr;
      if (!n->get_right(version, nullptr)->is_null)
      ...
      { // Caso n não tenha sub-árvore direita, o sucessor é o pai da sub-árvore cujo n é o máximo
  ...
  void print_to_file(int version, ofstream &file)
  {
    ...
    {
      char c;
      if (node->get_color(version, BLACK))
        c = 'R';
      else
        c = 'B';
  ...
  Node *search_successor(Node *node, int value, int version)
  {
    ...
    if (node->value > value)
    {
      ...
      if (!left->is_null)
        return search_successor(left, value, version);
    }
    if (node->value < value)
    {
      Node *right = node->get_right(version, nullptr);
      if (!right->is_null)
        return search_successor(right, value, version);
    }
    if (node->value == value)
    {
      Operator op(node);
    ...
    if (node->value < value)
      return node->get_parent(version, nullptr);
    return node;
  }
